...
function SocketPouch(opts, callback) {
  ...
  function sendMessage(type, args, callback) {
    ...
    api._socket.emit('message', type + ':' + messageId + ':' + stringArgs, function () {
  ...
  function sendBinaryMessage(type, args, blobIndex, blob, callback) {
    ...
    api._socket.emit('message', blobToSend, function () {
  ...
  api._changes = function (opts) {
    ...
    if (opts.continuous) {
      ...
      api._socket.emit('message', 'liveChanges' + ':' + messageId + ':' + JSON.stringify([opts]));
      return {
        cancel: function () {
          api._socket.emit('message', 'cancelChanges' + ':' + messageId + ':' + JSON.stringify([]));
        }
      };
    }
