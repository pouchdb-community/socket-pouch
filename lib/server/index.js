...
function sendUncaughtError(socket, data) {
  ...
  socket.emit('message', 'global:4:' + JSON.stringify(serverUtils.createError(data)));
}

function sendError(socket, messageId, data) {
  log(' -> sendError', socket.id, messageId, data);
  socket.emit('message', messageId + ':0:' + JSON.stringify(serverUtils.createError(data)));
}

function sendSuccess(socket, messageId, data) {
  log(' -> sendSuccess', socket.id, messageId);
  socket.emit('message', messageId + ':1:' + JSON.stringify(data));
}

function sendBinarySuccess(socket, messageId, type, buff) {
  log(' -> sendBinarySuccess', socket.id, messageId);
  var blobUuid = uuid();
  socket.emit('message', messageId + ':3:' + JSON.stringify({type: type, uuid: blobUuid}));
...
function sendUpdate(socket, messageId, data) {
  ...
  socket.emit('message', messageId + ':2:' + JSON.stringify(data));
}
